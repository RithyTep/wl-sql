export class SQLFormatter {
  format(input: string): string {
    const sql = input || '';
    const out = this.preserveSegments(sql, (code: string) => {
      let s = code;
  ...
  private bracketIdentifiers(sql: string): string {
    return sql.replace(/(?<!@)\b([a-zA-Z_][a-zA-Z0-9_]*(?:\.[a-zA-Z_][a-zA-Z0-9_]*)*)\b(?!\s*\()/g, (m: string) => {
      ...
      if (raw.includes('.')) {
        ...
        if (/^[a-z][a-z0-9_]*$/.test(first) && first.length <= 2) return `${first}.${rest}`;
  ...
  private addNoLock(sql: string): string {
    return sql.replace(/\b(FROM|INNER JOIN|JOIN|LEFT JOIN|RIGHT JOIN|FULL JOIN|CROSS JOIN)\s+([^\s(]+)(?:\s+([a-zA-Z0-9_]+))?/gi, (m: string, op: string, tblRaw: string, alias: string) => {
